...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	if (cell == nil) {
		NSArray *nib = [[NSBundle mainBundle] loadNibNamed:@"UserRequestsCustomCell" 
													 owner:self options:nil];
		
		cell = (UserRequestsCustomCell *)[nib objectAtIndex:0];
	}
    
	NSLog(@"reloading data");
    // Configure the cell...
	User *user = [currentUser.connectionRequestsFrom objectAtIndex:row];
	cell.customLabel.text = [NSString stringWithFormat:@"%@ %@", user.firstName, user.lastName];
